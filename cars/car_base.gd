...
func _physics_process(delta):
	...
	if $FrontWheelCast.is_colliding() or $RearWheelCast.is_colliding():
		var nf = $FrontWheelCast.get_collision_normal() if $FrontWheelCast.is_colliding() else Vector3.UP
		var nr = $RearWheelCast.get_collision_normal() if $RearWheelCast.is_colliding() else Vector3.UP
